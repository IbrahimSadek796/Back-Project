...
@section('content')
    ...
    <div class="page-content card">
        <form class="row g-3 card-body" action="{{ route('user.update', auth()->user()->id) }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card-title">
                <a class="float-end btn btn-primary mt-3" href="{{ route('user.') }}">Back</a>
            ...
            <div class="col-12">
                ...
                <input class="form-control @error('email') is-invalid @enderror" id="inputEmail" name="email"
                    type="text" value="{{ old('email',auth()->user()->email) }}" placeholder="Email...">
                @error('email')
                    <div class="alert alert-danger my-1">{{ $message }}</div>
                @enderror
            </div>
